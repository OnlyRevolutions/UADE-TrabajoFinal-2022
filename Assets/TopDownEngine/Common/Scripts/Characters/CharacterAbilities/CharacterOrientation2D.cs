			...
			if (_characterHandleWeapon.WeaponAimComponent != null)
			{
				switch (FacingBase)
				{
					...
					case FacingBases.MousePositionX:
						if (_characterHandleWeapon.WeaponAimComponent.GetMousePosition().x < transform.position.x)
						{
							FaceDirection(-1);
						}
						else
					...
					case FacingBases.SceneReticlePositionX:
						if (_characterHandleWeapon.WeaponAimComponent.GetReticlePosition().x < transform.position.x) 
						{
							FaceDirection(-1);
						}
						else
