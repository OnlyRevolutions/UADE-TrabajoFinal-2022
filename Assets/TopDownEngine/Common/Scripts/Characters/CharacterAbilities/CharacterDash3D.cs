		...
		protected virtual void HandleDashMode()
		{
			...
			switch (DashMode)
			{
				...
				case DashModes.ModelDirection:
					_dashDestination = transform.position + _characterOrientation3D.ModelDirection.normalized * DashDistance;
					break;
				...
				case DashModes.MousePosition:
					...
					angle = Vector3.SignedAngle(transform.forward, (_inputDirection - transform.position).normalized, Vector3.up);
					_dashDestination = transform.position + DashDirection.normalized * DashDistance;
					_dashAngle.y = angle;
					_dashDestination = MMMaths.RotatePointAroundPivot(_dashDestination, transform.position, _dashAngle);
