...
namespace MoreMountains.TopDownEngine
{
	public class DeadlineDoor : MonoBehaviour
	{
		...
		protected void OnTriggerEnter2D(Collider2D other)
		{
			...
			_newAngles = transform.localEulerAngles;
		...
		protected virtual IEnumerator RotateDoor(float currentAngle, float targetAngle)
		{
			...
			while (Time.time - startTime < DoorMovementDuration)
			{
				...
				transform.localEulerAngles = _newAngles;
				yield return null;
			}
		...
		{
			_vectorA = (transform.position + transform.right * _boxCollider2D.size.x) - transform.position;
			_vectorB = otherPosition - transform.position;
