...
{	
	...
	{
		private int InitialMaximumLives { get; set; }
		private int InitialCurrentLives { get; set; }
		...
		protected virtual void LevelComplete()
		{
			...
			{
				if (Scenes[i].SceneName != SceneManager.GetActiveScene().name) continue;
		...
		protected virtual void InitializeStars()
		{
			...
			{
				if (scene.SceneName != SceneManager.GetActiveScene().name) continue;
				
				int stars = scene.CollectedStars.Count(star => star);
		...
		protected virtual void SaveProgress()
		{
			...
			{
				StoredCharacterName = GameManager.Instance.StoredCharacter.name,
				MaximumLives = GameManager.Instance.MaximumLives,
				CurrentLives = GameManager.Instance.CurrentLives,
				InitialMaximumLives = InitialMaximumLives,
				InitialCurrentLives = InitialCurrentLives,
				Scenes = Scenes
			};
		...
		public virtual void OnMMEvent(TopDownEngineStarEvent deadlineStarEvent)
		{
			foreach (DeadlineScene scene in Scenes)
			{
				if (scene.SceneName != deadlineStarEvent.SceneName) continue;
