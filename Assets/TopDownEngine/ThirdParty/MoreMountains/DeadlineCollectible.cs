...
namespace MoreMountains.TopDownEngine
{
	...
	public class DeadlineCollectible : MonoBehaviour
	{
		...
		{
			ProgressManager.Instance.FindCollectible (CollectibleName);
		}
		...
		{
			if (ProgressManager.Instance.FoundCollectibles == null)
			{
				return;
			}
			foreach (string collectible in ProgressManager.Instance.FoundCollectibles)
			{
				if (collectible == CollectibleName)
				{
					Disable ();
				}
			}
